...
Clique no botão para parcipar.\n**Boa sorte!!!**`
      ...
      coletor.on("collect", (i) => {
        if (i.customId === "botao") {
          if (click.includes(i.user.id))
            return i.reply({
              content: `Olá ${interaction.user}, você ja está participando do sorteio.`,
              ephemeral: true,
            });
          ...
          i.reply({
            content: `Olá ${interaction.user}, você entrou no sorteio.`,
            ephemeral: true,
          });
        }
      });
      ...
      setTimeout(() => {
        ...
        interaction.followUp(
          `**Parabéns <@${ganhador}> você ganhou o sorteio: \`${premio}\`.**`
        );
      }, duracao);
    }
  },
