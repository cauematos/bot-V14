...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const embedUser = new EmbedBuilder()
      ...
      .addFields(
        {
          name: `<:users:1050929946261995630> Nome`,
          value: `\`\`\`${usuarioTest.tag}\`\`\``,
          inline: true,
        },
        {
          name: `<:id:1050930441105985646> Identidade`,
          value: `\`\`\`${usuarioTest.id}\`\`\``,
          inline: true,
        },
        {
          name: `<:Chat:1050931313244377108> Menção`,
          value: `<@${usuarioTest.id}>`,
          inline: true,
        },
        {
          name: `<:info:1050930856476282950> Conta Criada`,
    ...
    if (!acharUser)
      botaoUser.setLabel("Não encontrado no servidor"),
        botaoUser.setDisabled(true);
    if (acharUser)
      embedUser.addFields({
        name: `<:pinkcalend_:1050931751024873502> Entrou em`,
    ...
    coletorPerms.on("collect", async (interaction) => {
      if (interaction.customId === "verPerms") {
        ...
        const embedPerms = new EmbedBuilder().setColor("#5865f2").addFields(
          ...
          {
            name: `<:LKS_config:1049795709618114631> Permissões de ${usuarioTest.username}`,
