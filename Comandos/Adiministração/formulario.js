  ...
  run: async (client, interaction) => {
    if (
      ...
      const canal_formulario =
        interaction.options.getChannel("canal_formulário");
      ...
      if (canal_formulario.type !== Discord.ChannelType.GuildText) {
        ...
        interaction.reply({
          content: `O canal ${canal_logs} não é um canal de texto.`,
          ephemeral: true,
        });
      } else {
        await db.set(
          `canal_formulario_${interaction.guild.id}`,
          canal_formulario.id
        );
        ...
        interaction.reply({ embeds: [embed], ephemeral: true }).then(() => {
          let embed_formulario = new Discord.EmbedBuilder()
            ...
            .setDescription(
              `Caso queira virar um <@&1071160448730284185> preencha o \`formulario\`.`
            );
