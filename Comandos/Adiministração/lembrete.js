...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const embedS = new Discord.EmbedBuilder()
      .setColor("#0095ff")
      .setDescription(
        `**Olá, ${interaction.user}!** Lembrete setado com sucesso para acabar <t:${tempoR}:R>.`
      )
      .setFooter({
    ...
    const embedL = new Discord.EmbedBuilder()
      .setColor("#0095ff")
      .setDescription(
        `**Opa, ${interaction.user}!** Vim alertar-te sobre seu lembrete! Você pediu para ser lembrado sobre: \`${motivo}\`.`
      )
      .setFooter({
        text: `Comando requisitado por: ${interaction.user.tag}`,
        iconURL: interaction.user.displayAvatarURL({ format: "png" }),
